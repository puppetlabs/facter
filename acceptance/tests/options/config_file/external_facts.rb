test_name "external-dir and no-external-facts config fields allow control of external fact lookup"
# 
...
agents.each do |agent|
  os_version = on(agent, facter('kernalmajorversion')).stdout.chomp.to_f
  ...
  step "Agent #{agent}: set up facts.d and custom external fact directories"
...
EOM
  ...
  step "setting no-external-facts to true should disable external facts"
  ...
  step "setting external-dir should specify location of external facts"
