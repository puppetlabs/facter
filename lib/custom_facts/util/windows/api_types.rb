  ...
  module Util
    module Windows
      class FFI
        ...
        # @param max_char_length [Integer] Maximum number of wide chars to return
        #                                  (typically excluding NULLs), *not* bytes
        #
        # @param null_terminator [Symbol] Number of number of null wchar characters, *not* bytes,
        #                                 that determine the end of the string
        ...
        def self.read_arbitrary_wide_string_up_to(ffi_pointer, max_char_length = 512, null_terminator = :single_null)
          if null_terminator != :single_null && null_terminator != :double_null
            raise format(_('Unable to read wide strings with %<null_terminator>s terminal nulls'),
