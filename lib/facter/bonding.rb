...
} unless defined? BONDING_MODE

def parse_bonded_interface(interface_info)
  mutex = Mutex.new
  ...
  interface_info.each_line do |line|
    ...
    when /Bonding Mode:\s/
      ...
      mutex.synchronize do
        configuration.update(:mode => mode)
      end
    when /Primary Slave:\s/
      ...
      mutex.synchronize do
        configuration.update(:primary_slave => value)
      end
    when /Currently Active Slave:\s/
      ...
      mutex.synchronize do
        configuration.update(:active_slave => value)
      end
    when /MII Status:\s/
      # Take the value only ...
      value = line.split(':')[1].strip
      ...
      mutex.synchronize do
        configuration.update(:status => value)
      end
  ...
  mutex.synchronize do
    configuration.update(:slaves => slaves)
  end
  configuration
end
...
def bonding_detected?
  File.exists?('/proc/net/bonding')
end

def bonding_interfaces
  Dir['/proc/net/bonding/*']
end

def read_data_for_interfaces
  # We utilise rely on "cat" for reading values from entries under "/proc".
  # This is due to some problems with IO#read in Ruby and reading content of
  # the "proc" file system that was reported more than once in the past ...
  Array(bonding_interfaces).map { |interface| Facter::Util::Resolution.exec("cat #{interface} 2> /dev/null") }
end
...
if bonding_detected?
  Facter.add('bonding_interfaces') do
    ...
    setcode do
      Facter::Util::Resolution.exec("cat /sys/class/net/bonding_masters").split.join(",")
    end
  end

  #TODO use all the files in /sys/class/net/bond*/* and make a hash out of those.

  interface_data = read_data_for_interfaces
  interface_hashes = interface_data.map { |data| parse_bonded_interface(data) }
  interface_hashes.each do |interface|
