    ...
    class Partitions < BaseResolver
      ...
      class << self
        ...
        def populate_from_lsblk(partition_name, blkid_and_lsblk)
          ...
          # The -p/--paths option was added in lsblk 2.23, before that it's better return and fall back to blkid
          ...
          result[:part_type] = part_type if part_type
        ...
        def execute_and_extract_lsblk_info(lsblk_version)
          ...
          output_hash.each_value { |value_hash| value_hash.delete_if { |k,v| v.empty? } }
          output_hash.delete_if { |k,v| v.empty? }
        end
      end
    end
  end
