# frozen_string_literal: true
  ...
  module Facter
    module Resolvers
      module Linux
        class ProcessorsLscpu < BaseResolver
          init_resolver
          
          ITEMS = { threads_per_core: "| grep -e 'Thread(s)'",
            scores_per_socket: "-e 'Core(s)'"
          }.freeze
          ...
          class << self
            ...
            def read_cpuinfo(fact_name)
              lscpu_output = Facter::Core::Execution.execute("lscpu #{ITEMS.values.join(' ')}", logger: log)
