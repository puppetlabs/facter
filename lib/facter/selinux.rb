...
def selinux_mount_point
  ...
  if FileTest.exists?('/proc/self/mounts')
    ...
    selinux_line = Facter::Util::Resolution.exec('cat /proc/self/mounts 2> /dev/null').each_line.find { |line| line =~ /selinuxfs/ }
    if selinux_line
...
{ "selinux_current_mode" => "Current mode",
  ...
  Facter.add(fact) do
    ...
    setcode do
      result = 'unknown'
      mode = Facter::Util::Resolution.exec("#{sestatus_cmd} 2> /dev/null")
