  ...
  class ResolverManager
    def self.invalidate_all_caches
      ...
      resolver_class_names.each do |resolver_class_name|
        resolver_class = Class.const_get("Facter::Resolver::#{resolver_class_name}")

        resolver_class.invalidate_cache if resolver_class < Resolver::BaseResolver
      end
    end

    def self.discover_all_resolver_class_names
      constants_in_resolver = Module.const_get('Facter::Resolver')
