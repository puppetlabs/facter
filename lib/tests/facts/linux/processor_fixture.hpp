...
struct linux_cpu_fixture
{
    ...
    void set_physical_id(std::string const& physical_id);
    void set_speed(int64_t speed);
    std::string get_info();

private:
    void init_info(std::string const& model_name);
...
{
    explicit linux_processor_fixture(std::string const& root);
...
private:
    std::vector<linux_cpu_fixture> _cpus;
    int _next_id;
