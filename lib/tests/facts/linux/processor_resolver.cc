...
SCENARIO("determing the architecture of a linux machine") {
    ...
    GIVEN("that the isa fact was successfully calculated") {
        WHEN("it starts with ppc64, then POWER is returned for the machine's architecture") {
            test_data data;
        ...
        WHEN("it does not start with ppc64, then X86 is returned for the machine's architecture") {
            test_data data;
	...
	linux_processor_fixture fixture(root_dir, architecture_type::X86);
        WHEN("the /proc/cpuinfo file has the x86 structure, then X86 is returned for the machine's architecture") {
            fixture.reset();
        ...
        WHEN("the proc/cpu/info file almost has the power structure, then X86 is returned for the machine's architecture") {
        ...
        WHEN("the proc/cpu/info file has the power structure, then POWER is returned for the machine's architecture") {
            fixture.reset(architecture_type::POWER);
            vector<cpu_param> cpu_params({
                make_tuple("Model A", "0", "0", 10),
                make_tuple("Model B", "1", "1", 15),
