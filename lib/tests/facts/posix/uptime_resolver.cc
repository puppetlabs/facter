...
TEST(uptime_resolver, parse_executable_uptime) {
    ...
    // https://github.com/puppetlabs/facter/blob/2.0.1/spec/unit/util/uptime_spec.rb#L50-L98
    map<string, int> test_cases = {
    ...
    for (auto t : test_cases) {
        EXPECT_EQ(uptime_resolver::parse_executable_uptime(t.first), t.second);
    }
}
