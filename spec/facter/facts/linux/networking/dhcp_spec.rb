...
describe Facts::Linux::Networking::Dhcp do
  describe '#call_the_resolver' do
    ...
    before do
      allow(Facter::Resolvers::Linux::Networking).to receive(:resolve).with(:dhcp).and_return(value)
    end

    it 'calls Facter::Resolvers::NetworkingLinux with dhcp' do
      fact.call_the_resolver
      expect(Facter::Resolvers::Linux::Networking).to have_received(:resolve).with(:dhcp)
    end
