...
describe Facts::Macosx::SystemProfiler::ProcessorName do
  describe '#call_the_resolver' do
    ...
    it 'calls Facter::Resolvers::Macosx::SystemProfiler' do
      ...
      expect(Facter::Resolvers::Macosx::SystemProfiler).to have_received(:resolve).with(:processor_name)
    end

    it 'returns a resolved fact' do
      expect(fact.call_the_resolver).to be_an_instance_of(Array).and \
        contain_exactly(an_object_having_attributes(name: 'system_profiler.processor_name', value: value),
                        an_object_having_attributes(name: 'sp_processor_name', value: value, type: :legacy))
    end
  end
end
