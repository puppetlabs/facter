...
describe Facts::Openbsd::NetmaskInterfaces do
  subject(:fact) { Facts::Openbsd::NetmaskInterfaces.new }
  ...
  describe '#call_the_resolver' do
    let(:interfaces) { { 'em0' => { netmask: '10.255.255.255' }, 'vio0' => { netmask: '10.17.255.255' } } }
    ...
    it 'returns legacy facts with names netmask_<interface_name>' do
      expect(fact.call_the_resolver).to be_an_instance_of(Array).and \
        contain_exactly(an_object_having_attributes(name: 'netmask_em0',
                                                    value: interfaces['em0'][:netmask], type: :legacy),
                        an_object_having_attributes(name: 'netmask_vio0',
                                                    value: interfaces['vio0'][:netmask], type: :legacy))
    end
  end
