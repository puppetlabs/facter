...
describe Facts::Solaris::Networking::Dhcp do
  describe '#call_the_resolver' do
    subject(:fact) { Facts::Solaris::Networking::Dhcp.new }

    let(:value) { '10.16.122.163' }

    before do
      allow(Facter::Resolvers::Solaris::Networking).to receive(:resolve).with(:dhcp).and_return(value)
    end

    it 'calls Facter::Resolvers::NetworkingLinux with dhcp' do
      fact.call_the_resolver
      expect(Facter::Resolvers::Solaris::Networking).to have_received(:resolve).with(:dhcp)
    end
