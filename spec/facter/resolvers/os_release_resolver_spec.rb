...
describe 'OsReleaseResolver' do
  ...
  it 'returns os NAME' do
    result = Facter::Resolver::OsReleaseResolver.resolve('NAME')

    expect(result).to eq('Ubuntu')
  end

  it 'returns os PRETTY_NAME' do
    result = Facter::Resolver::OsReleaseResolver.resolve('PRETTY_NAME')

    expect(result).to eq('Ubuntu 18.04.1 LTS')
  end

  it 'returns os VERSION_ID' do
    result = Facter::Resolver::OsReleaseResolver.resolve('VERSION_ID')

    expect(result).to eq('18.04')
  end

  it 'returns os VERSION_CODENAME' do
    result = Facter::Resolver::OsReleaseResolver.resolve('VERSION_CODENAME')
