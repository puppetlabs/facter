...
describe Facter::Resolvers::Utils::Ffi::LoadAverages do
  describe '#load_averages' do
    let(:memory_pointer) { instance_spy(FFI::MemoryPointer) }

    before do
      allow(Facter::Resolvers::Utils::Ffi::LoadAverages).to receive(:getloadavg).and_return(response)
    ...
    context 'when averages are returned' do
      ...
      it 'returns load average' do
        expect(Facter::Resolvers::Utils::Ffi::LoadAverages.read_load_averages).to eq(averages)
      end
    end
