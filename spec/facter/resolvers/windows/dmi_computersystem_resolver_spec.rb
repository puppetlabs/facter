...
describe 'Windows DMIComputerSystemResolver' do
  ...
  after do
    Facter::Resolver::DMIComputerSystemResolver.invalidate_cache
  end

  context '#resolve' do
    let(:comp) { double('WIN32OLE', Name: 'VMware7,1', UUID: 'C5381A42-359D-F15B-7A62-4B6ECBA079DE') }
    it 'detects virtual machine name' do
      expect(Facter::Resolver::DMIComputerSystemResolver.resolve(:name)).to eql('VMware7,1')
    end
    it 'detects uuid of virtual machine' do
      expect(Facter::Resolver::DMIComputerSystemResolver.resolve(:uuid)).to eql('C5381A42-359D-F15B-7A62-4B6ECBA079DE')
    end
  end
  ...
  context '#resolve when WMI query returns nil' do
    ...
    it 'logs debug message and name is nil' do
      ...
      expect(Facter::Resolver::DMIComputerSystemResolver.resolve(:name)).to eql(nil)
    end
    it 'detects uuid as nil' do
      expect(Facter::Resolver::DMIComputerSystemResolver.resolve(:uuid)).to eql(nil)
    end
  end
