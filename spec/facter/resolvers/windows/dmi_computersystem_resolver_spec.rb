...
describe 'Windows DMIComputerSystemResolver' do
  ...
  after do
    DMIComputerSystemResolver.invalidate_cache
  end
  ...
  context '#resolve when WMI query returns nil' do
    ...
    it 'logs debug message and name is nil' do
      ...
      expect(DMIComputerSystemResolver.resolve(:name)).to eql(nil)
    end
    it 'detects uuid as nil' do
      expect(DMIComputerSystemResolver.resolve(:uuid)).to eql(nil)
    end
  end
