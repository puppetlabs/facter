...
describe 'Windows IdentityResolver' do
  ...
  after do
    Facter::Resolver::IdentityResolver.invalidate_cache
  end
  ...
  context '#resolve when user is administrator' do
    ...
    it 'detects user' do
      expect(Facter::Resolver::IdentityResolver.resolve(:user)).to eql('MG93C9IN9WKOITF\Administrator')
    end
    it 'detects that user is administrator' do
      expect(Facter::Resolver::IdentityResolver.resolve(:privileged)).to eql(true)
    end
  end
  ...
  context '#resolve when user is not administrator' do
    ...
    it 'detects user' do
      expect(Facter::Resolver::IdentityResolver.resolve(:user)).to eql('MG93C9IN9WKOITF\User')
    end
    it 'detects that user is not administrator' do
      expect(Facter::Resolver::IdentityResolver.resolve(:privileged)).to eql(false)
    end
  end
  ...
  context '#resolve when' do
    ...
    it 'could not determine if user is admin' do
      expect(Facter::Resolver::IdentityResolver.resolve(:privileged)).to eql(nil)
    end
  end
  ...
  context '#resolve when error code is different than ERROR_MORE_DATA' do
    ...
    it 'logs debug message when trying to resolve user' do
      ...
      expect(Facter::Resolver::IdentityResolver.resolve(:user)).to eql(nil)
    end
