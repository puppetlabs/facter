...
describe 'Windows VirtualizationResolver' do
  ...
  context '#resolve VirtualBox' do
    after do
      Facter::Resolver::VirtualizationResolver.invalidate_cache
    end
    ...
    it 'detects virtual machine model' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:virtual)).to eql('virtualbox')
    end
    it 'detects that is virtual' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:is_virtual)).to eql('true')
    end
  end
  ...
  context '#resolve Vmware' do
    ...
    it 'detects virtual machine model' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:virtual)).to eql('vmware')
    end
    it 'detects that is virtual' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:is_virtual)).to eql('true')
    end
  ...
  context '#resolve KVM' do
    ...
    it 'detects virtual machine model' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:virtual)).to eql('kvm')
    end
    it 'detects that is virtual' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:is_virtual)).to eql('true')
    end
  ...
  context '#resolve Openstack VM' do
    ...
    it 'detects virtual machine model' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:virtual)).to eql('openstack')
    end
    it 'detects that is virtual' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:is_virtual)).to eql('true')
    end
  ...
  context '#resolve Microsoft VM' do
    ...
    it 'detects virtual machine model' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:virtual)).to eql('hyperv')
    end
    it 'detects that is virtual' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:is_virtual)).to eql('true')
    end
  ...
  context '#resolve Xen VM' do
    ...
    it 'detects virtual machine model' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:virtual)).to eql('xen')
    end
    it 'detects that is virtual' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:is_virtual)).to eql('true')
    end
  ...
  context '#resolve Physical Machine' do
    ...
    it 'detects virtual machine model' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:virtual)).to eql('physical')
    end
    it 'detects that is not virtual' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:is_virtual)).to eql('false')
    end
  end
  ...
  context '#resolve  when WMI query returns nil' do
    ...
    it 'logs that query failed and virtual nil' do
      ...
      expect(Facter::Resolver::VirtualizationResolver.resolve(:virtual)).to eql(nil)
    end
    it 'detects that is_virtual nil' do
      expect(Facter::Resolver::VirtualizationResolver.resolve(:is_virtual)).to eql(nil)
    end
  end
