...
describe "macaddress_path" do
  ...
  describe "on linux" do
    it "should return /sbin/ip if it's on the server" do
      ...
      Facter.fact(:macaddress_path).value.should == '/sbin/ip'
    end
    ...
    it "should return /sbin/ifconfig if /sbin/ip isn't there" do
      ...
      Facter.fact(:macaddress_path).value.should == '/sbin/ifconfig'
    end
    ...
    it "should fail if neither /sbin/ifconfig or /sbin/ip are available" do
      ...
      Facter.fact(:macaddress_path).value.should == nil
    end
  end
  ...
  describe "on sunos" do
    it "should return /usr/sbin/ifconfig" do
      ...
      Facter.fact(:macaddress_path).value.should == '/usr/sbin/ifconfig'
    end
  end

  describe "on hp-ux" do
    it "should return /sbin/lanscan" do
      Facter.fact(:kernel).stubs(:value).returns(:"hp-ux")
      FileTest.stubs(:executable?).with('/sbin/ip').returns(false)
      FileTest.stubs(:executable?).with('/sbin/ifconfig').returns(false)
      FileTest.stubs(:executable?).with('/usr/sbin/ifconfig').returns(false)
      FileTest.stubs(:executable?).with('/sbin/lanscan').returns(true)

      Facter.fact(:macaddress_path).value.should == '/sbin/lanscan'
    end
  end
  ...
  describe "on windows" do
    it "should return netsh.exe" do
      ...
      Facter.fact(:macaddress_path).value.should == "#{ENV['SYSTEMROOT']}/system32/netsh.exe"
    end
  end
end
