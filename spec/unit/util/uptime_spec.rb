        ...
        describe "should use 'uptime' command" do
          ...
          test_cases.each do |uptime, expected|
            it "should return #{expected} for #{uptime}" do
              Facter::Util::Resolution.stubs(:exec).with('/usr/bin/uptime 2>/dev/null').returns(uptime)
          ...
          describe "nor is 'uptime' command" do
            before :each do
              Facter::Util::Uptime.stubs(:uptime_uptime_cmd).returns("cat \"#{@nonexistent_file}\"")
            end
